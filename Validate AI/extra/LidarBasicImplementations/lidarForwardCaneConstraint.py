...
class CaneEnv(gym.Env):
    def __init__(self):
        ...
        p.createConstraint(
            parentBodyUniqueId=self.cane_id,  # Cane body ID
            parentLinkIndex=-1,  # -1 if attaching to base; otherwise, use the correct link index
            childBodyUniqueId=self.lidar_id,  # LiDAR body ID
            childLinkIndex=-1,
            jointType=p.JOINT_FIXED,
            jointAxis=[0, 0, 0],
            parentFramePosition=[0, 0, 0],  # Adjust this relative to the cane
            childFramePosition=[0, 0, 0]  # Adjust this relative to the LiDAR
        )
    ...
    def get_lidar_data(self):
        ...
        # Calculate the position of the LiDAR beam
        lidar_offset = [self.cane_radius, 0, -self.cane_height / 3]  
        ...
        # Calculate the direction of the beam (rotate 90 degrees to the left)
        direction = [-np.sin(cane_yaw), np.cos(cane_yaw), -1]
        #direction = [-np.sin(cane_yaw), np.cos(cane_yaw), 0]  # Keep it horizontal
    ...
    def step(self, action):
        ...
        cane_pos, cane_ori = p.getBasePositionAndOrientation(self.cane_id)
        lidar_offset = [0, 0, 0.2]  # Adjust based on actual LiDAR placement
        lidar_pos = p.multiplyTransforms(cane_pos, cane_ori, lidar_offset, [0, 0, 0, 1])[0]

        p.resetBasePositionAndOrientation(self.lidar_id, lidar_pos, cane_ori)
