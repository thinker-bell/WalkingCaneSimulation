...
class CaneEnv(gym.Env):
    ...
    def reset_lidar(self):
        ...
        direction = [np.cos(cane_yaw), np.sin(cane_yaw), 0]
        step_size = 0.3
        num_steps = 2
        lidar_pos = [cane_pos[0] + num_steps * step_size * direction[0],
                    cane_pos[1] + num_steps * step_size * direction[1],
                    cane_pos[2] + 0.1]  # Adjusted the z-coordinate to mount the cube a little higher
        lidar_orientation = p.getQuaternionFromEuler([np.pi/2, 0, cane_yaw])
    ...
    def get_lidar_data(self):
        ...
        direction = [np.cos(lidar_yaw), np.sin(lidar_yaw), -1]
